...
// by side, one cold/warm column pair per vitest version.
...
for (const app of apps) {
  ...
  const columns = states.flatMap(state => versions.map(version => column(version, state)))

  console.log(`### ${app}\n`)
  console.log(`| ${dimensions.map(d => d.name).join(' | ')} | ${columns.join(' | ')} |`)
  console.log(`|${dimensions.map(() => '---').join('|')}|${columns.map(() => '---:').join('|')}|`)
  for (const entry of entries) {
    console.log(`| ${dimensions.map(d => d.get(entry)).join(' | ')} | ${columns.map(name => entry[name] ?? '—').join(' | ')} |`)
  }
  console.log()
}
